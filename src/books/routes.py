...
from src.books.schemas import Book, BookUpdateModel, BookUpdateResponseModel, BookDeleteResponseModel
...
) -> List[Book]: #> Changed Optional[List[Book]] to List[Book] since it returns an empty list [] if none found, not None
...
async def create_a_book(
    book_data: Book, session: Annotated[AsyncSession, Depends(get_session)]
) -> Book:
...
async def update_book(
    ...
    old_book_data = current_book.model_copy() # Creates a new Pydantic instance with old data
...
#! --- DELETE BOOK ---
@book_router.delete("/{book_uid}", status_code=status.HTTP_200_OK, response_model=BookDeleteResponseModel)
